...
public class ResponseDecoder implements ChainDecoder {
    ...
    private ResponseModel decode(Dg dg) {
        ...
        int channelIndex = CommonMethod.toShort(Arrays.copyOfRange(model.getBody(), 0, 2));
        int seq = ByteBuffer.wrap(Arrays.copyOfRange(model.getBody(), 2, 2 + 4)).getInt();
        model.setSeq(seq);
        CommonState state = CommonState.getByCode(CommonMethod.toByte(Arrays.copyOfRange(model.getBody(), 2 + 4, 2 + 4 + 1)));
        model.setState(state);
        if (!Objects.equals(state, CommonState.SUCCESS_NO_RES)) {
            int length = CommonMethod.toShort(Arrays.copyOfRange(model.getBody(), 2 + 4 + 1, 2 + 4 + 1 + 2));
    ...
    public ChainDecoder selected(DecoderType type) {
        return Objects.equals(type, DecoderType.RES) ? this : null;
    }
}
